...
export const removeOrphanedInvalidation = async (queryContext: QueryContext, table: string) => {
  ...
  const raw2Sql = ` DELETE FROM invalidation 
    WHERE invalidation.id NOT IN(select parent_id from invalidation_timeliness WHERE parent_id=invalidation.id)`
...
export const findOneLinkedInvalidation = async (queryContext: QueryContext, invalidationId: number) => {
  const { dataSource, timeRange } = queryContext;
  const rawSql = `SELECT parent_id from invalidation_timeliness where parent_id = ${invalidationId}`
  return dataSource.query(rawSql, timeRange);
}
export const removeOrphanedInvalidationTimeliness = async (queryContext: QueryContext, table: string) => {
  const { dataSource, timeRange } = queryContext;
  const rawSql =
    `DELETE FROM invalidation_timeliness WHERE product_ids ='{}'
  `;
  return dataSource.query(rawSql, timeRange);
};
