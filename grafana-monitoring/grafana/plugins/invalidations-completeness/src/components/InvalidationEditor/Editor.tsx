...
async function createInvalidation(formData: FormDTO, queryContext: QueryContext, productIds: number[]) {
  ...
  const table = `invalidation_completeness`;
  const rawSql =
    `with created_id as (
...
export const SelectContext = React.createContext(SELECTOPTIONS.COMPLETENESS);
...
export const Editor: FC<Props> = ({ selectedRows, onClose, unSelect }) => {
  ...
  const [defaultRootCause] = React.useState<SelectableValue<string>>(initialDefaultRootCause ? options.rootCauseListCompleteness.filter((f) => f.value === initialDefaultRootCause)[0] : SELECTOPTIONS.COMPLETENESS.rootCause.options[0]);
  ...
  const selectOption = options.rootCauseListCompleteness;
