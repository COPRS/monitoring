...
import { Checkbox, Input, Label, stylesFactory, useTheme, VerticalGroup } from '@grafana/ui';
...
export const FilterList: FC<Props> = ({ options, values, onChange }) => {
  ...
  const height = useMemo(() => Math.min(items.length * ITEM_HEIGHT, MIN_HEIGHT) + gutter, [gutter, items]);
